...
说明：
- 读取 `{输出目录}/接口规格.md` 文件（如果用户指定了输出目录）
- 或读取当前目录下的 `接口规格.md` 文件（如果用户未指定输出目录）
- 解析文件中的"组件接口说明"章节
...
工作流程：
1. 确定输入文件路径：
   - 如果用户指定了输出目录，读取 `{输出目录}/接口规格.md`
   - 如果用户未指定，读取当前目录下的 `接口规格.md`
2. 检查文件是否存在
3. 如果不存在，提示："接口规格.md 文件不存在，请先执行需求提取流程"
4. 如果存在，解析文件中的"组件接口说明"章节
5. 识别OpenAPI V3.0格式的接口定义（YAML格式）
6. 提取每个接口的完整信息：
...
说明：
- **所有文件必须写入到用户指定的输出目录**
- 测试数据输出文件：`{输出目录}/接口测试数据.yaml`（必须使用YAML格式）
- 测试脚本输出文件：`{输出目录}/接口测试脚本.robot`
- 文件结构清晰，便于维护
...
文件命名规则：
- 测试数据：`{输出目录}/接口测试数据.yaml`（必须使用YAML格式）或按接口拆分：`{输出目录}/{接口名称}-测试数据.yaml`
- 测试脚本：`{输出目录}/接口测试脚本.robot` 或按接口拆分：`{输出目录}/{接口名称}-测试脚本.robot`
...
反馈格式：
- 成功：`成功生成 [N] 个接口的测试数据，共 [M] 个测试用例，已保存到 {输出目录}/接口测试数据.yaml 和 {输出目录}/接口测试脚本.robot`
...
2. ✅ [规则 1] 从接口规格.md读取接口定义（关键）
3. ✅ [规则 2] OpenAPI V3.0接口定义解析（关键）
4. ✅ [规则 3] 测试数据生成规范（关键）
5. ✅ [规则 4] RobotFramework测试脚本生成规范（关键）
6. ✅ [规则 5] 测试数据到脚本的映射规则（高）
7. ✅ [规则 6] 文件生成规则（中）
8. ✅ [规则 7] 验证和完整性检查（高）
9. ✅ [规则 8] 输出反馈和统计（低）
...
3. 验证目录是否存在，如不存在则创建
4. 读取 {输出目录}/接口规格.md 文件
5. 解析"组件接口说明"章节
6. 识别OpenAPI V3.0 YAML格式的接口定义
7. 解析每个接口：
   - 接口概述
   - 入参定义
   - 返回定义
8. 为每个接口的每个字段生成测试数据：
   ...
   - 特殊字符（负例）
   - 安全注入（负例）
9. 生成测试数据文件：{输出目录}/接口测试数据.yaml（YAML格式）
10. 为每个测试数据生成RobotFramework测试用例
11. 生成测试脚本文件：{输出目录}/接口测试脚本.robot
12. 验证完整性
13. 输出："成功生成 3 个接口的测试数据，共 45 个测试用例，已保存到 {输出目录}/接口测试数据.yaml 和 {输出目录}/接口测试脚本.robot"
```
...
# 版本历史
# ============================================
# v1.1 (2025-01-XX) - 新增输出目录管理规则，要求用户明确指定输出目录，所有生成物均以该目录为根目录
